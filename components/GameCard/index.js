...
const GameCard = ({
	...
	return (
		...
		>
			<div>
				<img
					className="rounded-t-lg w-full h-40 object-contain bg-gray-700"
					src={imgSrc}
					alt="Card image cap"
				/>
				<div className="px-4 py-2 mt-2 text-gray-200">
					...
					<div className="flex items-center mb-4 space-x-1">
						{tags.map((tag, index) => (
							<span
								key={index}
								className="text-sm font-bold capitalize rounded bg-purple-500 text-gray-900 px-1.5 py-0"
							>
								{tag}
							</span>
						))}
			...
			<div className="px-5 mb-6 text-right">
				<Link href={"game/" + JSON.stringify(id)}>
					<a className="bg-pink-500 font-bold text-sm group-hover:bg-pink-600 duration-400 transition-all text-white rounded-md px-3 py-1.5">
						Play Now!
					</a>
				</Link>
			</div>
